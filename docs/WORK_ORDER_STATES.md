...
| `READY_TO_COMPLETE` | 三级故障、措施、维修完成附件、实际用件齐备 | 人工确认提交 |
| `COMPLETED` | 瑞云维修节点完成且同步成功 | 进入返程发货 |
| `REWORK` | 完工前校验失败或授权返工 | 回 `IN_PROGRESS` |

服务单进入后的固定顺序为：先将负责人改派给提交该工单的师傅；再确认“保外转保内”（普通保内、普通保外付费维修均填“否”，只有厂家特殊申请且有申请单号时填“是”）；随后添加实际配件。添加配件时仅按物料编码选择新件并核对数量，默认数量为 1，师傅实际更换多件时才修改数量，其余字段不改。

维修措施由固定话术模板和 `CONSUMED` 的实际用件生成。申请中、已发料但未使用或已归还的配件不能进入措施。责任判定不填写。维修完成照片和视频必须上传到服务单普通“附件”后才能完工。

瑞云配件的“是否返厂”为“是”时记录旧件标签待打印。标签只在完工后打印：勾选本单全部需返厂配件后统一点击“旧件打印标签”；无需返厂的配件不得勾选。打印机型号和正式打印配置待确认。

### 3.8 完工 `completion_status`

`DRAFT → VALIDATED → CONFIRM_PENDING → SYNCING → COMPLETED`。校验必须覆盖三级故障、维修措施、质保状态、维修完成照片/视频、用件一致性、服务单状态和操作者权限；责任判定保持空白。失败进入 `VALIDATION_FAILED` 或 `SYNC_FAILED`，可修正/幂等重试；不得因前端显示成功而视为完成。当前所有同步动作禁用。
...
| 提交完工 | 负责人、保外转保内结果、三级故障、措施、维修完成照片/视频和实际用件齐全，人工确认 | 否 |
