...
3. 创建后点击“下载预配置安装包”。已有终端点击“安装到新电脑”，确认后生成新密钥，旧密钥立即失效；正在打印时禁止换机。先退出旧电脑的助手，切勿将同一包同时安装到多台电脑。
4. 在 Windows 解压全部文件，双击 `Install.cmd`，允许管理员授权。服务器地址和终端密钥已内置，无需填写；找不到预设打印机时按提示选择。安装后在网页确认在线并测试打印。下载包及解压目录含密钥，安装完成后删除，不要转发或提交 Git。

以下命令仅供维护人员手动安装使用，普通用户不需要运行：
...
- 当前已确认纸张为 76×130 mm。原标签旋转后等比放大 120% 为 72×96 mm，居中打印（左右各留 2 mm，上下各留 17 mm；PDF 自身白边仍保留）。服务端转换为 576×768 单色点阵，并再次解码核对两个原始条码；不通过则停止。通过 TSPL BITMAP 把原图交给 Windows RAW 通道，每张先 CLS 清空打印缓冲区。正式标签不包含诊断标记。每页在表格外空白处放大印上任务所属师傅账号的数字部分（例如 `FieldDesk0005` 印为 `0005`，保留前导零），不印 `FieldDesk` 前缀或姓名，也不取打印电脑的登录人；无有效账号则停止入队。
