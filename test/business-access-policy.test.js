...
test("only owner bypasses business restrictions; test account follows technician permissions", () => {
  ...
  assert.equal(isBusinessRuleExempt(recloudTest), false);
  assert.equal(hasBusinessRole(owner, "TECHNICIAN"), true);
  assert.equal(hasBusinessRole(recloudTest, "ADMIN"), false);
  assert.equal(hasBusinessRole(recloudTest, "TECHNICIAN"), true);
...
test("frontend matches owner-only business exemption", async () => {
  ...
  assert.equal(frontendPolicy.isBusinessRuleExempt({ id: "FieldDesk0004" }), false);
...
test("protected account governance remains reserved for the owner", async () => {
  ...
  assert.doesNotMatch(userStore, /isRecloudTestAccount\(user\).*page !== "accountManagement"/s);
