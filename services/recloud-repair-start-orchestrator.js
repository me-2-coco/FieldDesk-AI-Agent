...
async function orchestrateRepairStart(payload, adapter, options = {}) {
  const timed = (phase, operation) => timeRecloudPhase(options.orderKey || 'UNKNOWN', `preparation_${phase}`, operation);
  ...
  let assignee = await timed('read_assignee', () => adapter.readAssignee());
  ...
  if (assignmentRequired) {
    ...
    const assignment = await timed('assign', () => adapter.assignResponsible(assignmentPlan));
    ...
    assignee = await timed('verify_assignee', () => adapter.readAssignee());
  ...
  let remote = await timed('parts_initial_read', readPartsState);
  ...
  await timed('warranty_conversion', () => adapter.confirmWarrantyConversion({
  ...
  remote = await timed('parts_precheck', readPartsState);
  ...
  if (partsPlan.additions.length) {
    ...
    const addResult = await timed('add_parts', () => adapter.addParts(partsPlan.additions, {
    ...
    remote = await timed('parts_postcheck', readPartsState);
    partsPlan = buildRecloudRepairPartsPlan(payload.usedParts, remote.parts);
