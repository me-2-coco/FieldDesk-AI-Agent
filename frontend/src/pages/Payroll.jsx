...
const tabs = [{ key: 'rows', label: '已计薪' }, { key: 'repeatRepairs', label: '重复维修' }, { key: 'duplicates', label: '重复已排除' }, { key: 'pending', label: '待核对' }]
...
export default function Payroll({ setPage }) {
  ...
  return <main className="payroll-page">
    ...
    <p className="payroll-note">北京时间自然月：每月1日00:00起，至次月1日00:00前。同一寄修单只计一次。同SN一个月内再次维修且本次换配件不计薪，跨月、跨师傅均核对；未换配件正常计薪。</p>
    ...
    {ready && <>
      ...
      <section className="payroll-totals" aria-label="本月汇总">
        ...
        <div><span>计薪维修台数</span><strong>{report.summary.repair}</strong><small>洗地机 ¥15 / 扫地机 ¥30</small></div>
        <div><span>计薪弃修台数</span><strong>{report.summary.abandoned}</strong><small>洗地机 ¥5 / 扫地机 ¥10</small></div>
        <div><span>重复维修（不计薪）</span><strong>{report.summary.repeatRepairs || 0} 次</strong><small>扣除 ¥{money(report.summary.deduction || 0)}</small></div>
      </section>
      <p className="payroll-note">扣除前 ¥{money(report.summary.grossAmount ?? report.summary.amount)} − 重复维修扣除 ¥{money(report.summary.deduction || 0)} = 应结工资 ¥{money(report.summary.amount)}。计薪台数已排除重复维修，不会再扣一次。</p>
      ...
      <section className="payroll-panel"><div className="payroll-section-title"><div><h2>师傅工资表</h2><p>点击师傅或“查看明细”，核对台数和对应工单。</p></div><span>共计 {report.summary.total} 台</span></div>
        <div className="payroll-scroll"><table><thead><tr><th>师傅 / 账号</th>{report.rates.map(rate => <th key={rate.key}>{rate.product}{rate.category}<small>台数 × ¥{rate.price}</small></th>)}<th>计薪维修 / 弃修 / 合计</th><th>重复维修 / 扣除</th><th>工资合计</th><th>核对 / 明细</th></tr></thead><tbody>
          {report.people.map(p => <tr key={p.technicianId} className={selected === p.technicianId ? 'payroll-selected' : ''}><td><button className="payroll-link" onClick={() => selectPerson(p.technicianId)}>{p.technicianName}</button><small>{p.technicianId || '未归属'}</small></td>{report.rates.map(rate => <td key={rate.key}>{p.counts[rate.key]} × {rate.price}<small>¥{money(p.counts[rate.key] * rate.price)}</small></td>)}<td>{p.repair} / {p.abandoned} / {p.total}</td><td>{p.repeatRepairs || 0} 次<small>扣除 ¥{money(p.deduction || 0)}</small></td><td className="payroll-amount">¥{money(p.amount)}<small>扣除前 ¥{money(p.grossAmount ?? p.amount)}</small></td><td><button className="payroll-link" onClick={() => selectPerson(p.technicianId)}>查看明细</button><small>待核对 {p.pending} · 重复 {p.duplicates}</small></td></tr>)}
          {!report.people.length && <tr><td colSpan="9" className="payroll-empty">本月暂无师傅或完工记录。</td></tr>}
        </tbody><tfoot><tr><th>合计</th>{report.rates.map(rate => <td key={rate.key}>{report.summary.counts[rate.key]} 台</td>)}<td>{report.summary.repair} / {report.summary.abandoned} / {report.summary.total}</td><td>{report.summary.repeatRepairs || 0} 次<small>扣除 ¥{money(report.summary.deduction || 0)}</small></td><td>¥{money(report.summary.amount)}</td><td>同单重复 {report.summary.duplicates} 条</td></tr></tfoot></table></div>
      </section>
      <section className="payroll-panel" aria-label="工单明细"><div className="payroll-section-title"><div><h2>{person ? `${person.technicianName}的明细` : '全部师傅明细'}</h2><p>{person ? `${person.technicianId || '未归属'} · 维修 ${person.repair} 台 + 弃修 ${person.abandoned} 台 = ${person.total} 台 · ¥${money(person.amount)}` : '可按寄修单号、机器 SN 或师傅账号搜索。'}</p></div>{selected !== null && <button onClick={() => selectPerson(null)}>查看全部师傅</button>}</div>
        ...
        <p className="payroll-note">{tab === 'repeatRepairs' ? '同一SN在前次完工后一个月内再次维修，且本次更换配件。本次不结算，前次工资不倒扣。一个月到次月同日同一时刻（含），月底无对应日期取月末。' : tab === 'duplicates' ? '以下记录已排除，不会重复计入台数或工资；核对说明列出首次提交时间及账号。' : tab === 'pending' ? '以下记录尚未计薪，修正源工单后刷新核算。' : '每行一台，计薪金额按机型和维修分类计算。'}</p>
        <div className="payroll-scroll"><table><thead><tr><th>寄修单号 / SN</th><th>师傅 / 账号</th><th>机型</th><th>维修类型</th><th>提交完工时间</th><th>单价</th><th>计薪金额</th>{tab === 'repeatRepairs' && <><th>前次工单 / 完工时间 / 师傅</th><th>本次更换配件</th><th>未结算金额</th></>}{tab !== 'rows' && <th>核对说明</th>}</tr></thead><tbody>{records.slice((visiblePage - 1) * 30, visiblePage * 30).map(row => <tr key={row.rowId}><td>{row.rmaNo || '缺少单号'}<small>{row.sn || '未记录SN'}</small></td><td>{row.technicianName}<small>{row.technicianId || '未归属'}</small></td><td>{row.product || '未分类'}<small>{row.model}</small></td><td>{row.treatment}<small>计薪分类：{row.category}</small></td><td>{row.completedTime}<small>北京时间</small></td><td>{row.unitPrice === null ? '待确认' : `¥${money(row.unitPrice)}`}</td><td>{row.amount === null ? '暂不计薪' : `¥${money(row.amount)}`}</td>{tab === 'repeatRepairs' && <><td>{row.previousRmaNo}<small>{row.previousCompletedTime}</small><small>{row.previousTechnicianName} · {row.previousTechnicianId}</small></td><td className="payroll-reason">{row.parts.map(p => `${p.name || p.code} × ${p.quantity}`).join('；')}</td><td>¥{money(row.deduction)}</td></>}{tab !== 'rows' && <td className="payroll-reason">{row.reason}</td>}</tr>)}{!records.length && <tr><td colSpan={tab === 'rows' ? 7 : tab === 'repeatRepairs' ? 11 : 8} className="payroll-empty">没有符合条件的记录。</td></tr>}</tbody></table></div>
      ...
      </section><p className="payroll-note">当前为实时核算，未锁账或标记工资发放。导出包含全部师傅汇总、计薪明细、重复维修及未结算金额、重复记录、待核对记录和核算说明，不受明细搜索条件影响。</p>
    </>}
  </main>
}
