...
export default function MonthlyStatistics() {
  ...
  return <section className="monthly-statistics card">
    ...
    {data && <>
      ...
      <div className="monthly-toolbar"><div><h2>人员汇总</h2><small>仅正式账号 · 列表可上下滑动</small></div>{data.canExport && <button type="button" className="monthly-export" title="导出汇总、工单与配件明细，WPS 可打开" disabled={exporting || !rows.length} onClick={exportDetails}>{exporting ? "导出中…" : "↓ 导出表格"}</button>}</div>
