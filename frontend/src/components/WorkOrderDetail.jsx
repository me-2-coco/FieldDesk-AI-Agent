...
export default function WorkOrderDetail({ order, onBack }) {
  return <div className="page machine-tracking-page">
    ...
    <section className="card">
      ...
      {order.hold && <section><h3>暂存记录</h3><p>{order.hold.category} / {order.hold.reason}</p><p>{order.hold.remark}</p><p>瑞云同步：{({ CONFIRMED: "已同步", FAILED: "失败待重试", PENDING: "待同步", SUBMITTING: "同步中", RESULT_UNKNOWN: "结果待核对" })[order.hold.status] || "待核对"}</p></section>}
