...
export default function HomeTodos({onOpen, technician}) {
  ...
  return <section className="card home-todos">
    ...
    {data && <><div className="todo-grid">{data.groups.map(g=><button type="button" key={g.id} aria-pressed={activeGroup===g.id} className={`todo-category category-${g.id} ${activeGroup===g.id?'active':''}`} onClick={()=>setSelected(g.id)}><span>{g.label}</span><strong>{g.count}<small> 单</small></strong></button>)}</div>
      ...
      {!!items.length && <div className="todo-list" aria-label={`${label}列表`} key={activeGroup}>{items.map(i=><button type="button" className={`todo-message category-${i.group}`} key={i.id} onClick={()=>onOpen(i)}><span><small className="todo-message-tag">{data.groups.find(g=>g.id===i.group)?.label}</small><strong>{i.rmaNo || '后台任务'}</strong><small>负责师傅：{i.technicianName || '未记录'}</small><em>{i.message}</em><small className="todo-next-action">下一步：{i.action || '查看详情'}</small></span><b aria-hidden="true">›</b></button>)}</div>}
    </>}
  </section>
}
