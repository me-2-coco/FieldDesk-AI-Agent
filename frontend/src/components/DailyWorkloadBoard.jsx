...
export default function DailyWorkloadBoard({ orders, technicians, user, now, loading, error }) {
  ...
  const board = boards[productIndex]
  ...
  return <section className="daily-workload-board" aria-label="当日维修看板">
    ...
    <div className="daily-product-tabs" role="group" aria-label="选择品类">{boards.map((board, index) => <button key={board.product} type="button" aria-pressed={index === productIndex} onClick={() => { setProductIndex(index); setPage(0) }}>{board.product}</button>)}</div>
    {loading ? <p role="status">正在加载当天工作量…</p> : error ? <p role="alert">工作量加载失败：{error}，正在自动重试</p> : <>
      <section className={`daily-summary ${productIndex ? "daily-summary-wash" : ""}`} aria-label={`${product}当日汇总`}>
        <div className="daily-summary-total"><div><span>今日已完成</span><p><strong>{totals.total}</strong> 台</p></div><span>{people.length} 位师傅</span></div>
        <div className="daily-summary-metrics">{columns.map(([key, label]) => <div key={key}><strong>{totals[key]}</strong><span>{label}</span></div>)}</div>
      </section>
      <div className="daily-list-heading"><h2>师傅明细</h2><span>{filtered.length} 人</span></div>
